...
public class TeacherRegistration extends javax.swing.JPanel {
    ...
    private void LoadTeachers() {
        ...
        if (!jButton4.isEnabled()) {
            ...
            try {
                ...
                String query = "SELECT * FROM `teachers` INNER JOIN `gender` ON teachers.gender_id = gender.id INNER JOIN `status` ON "
                        + "teachers.status_id=status.id WHERE teachers.`nic` LIKE '" + jTextField1.getText() + "%' AND NOT "
                        + "teachers.`type_id`='1' ORDER BY ";

                String sort = String.valueOf(jComboBox3.getSelectedItem());

                if (sort == "T_No ASC") {
                    query += "teachers.`tno` ASC";
                } else if (sort == "T_No DESC") {
                    query += "teachers.`tno` DESC";
                } else if (sort == "NIC ASC") {
                    query += "teachers.`nic` ASC";
                } else if (sort == "NIC DESC") {
                    query += "teachers.`nic` DESC";
                ...
                while (resultSet.next()) {
                    ...
                    Vector<String> vector = new Vector<>();
                    vector.add(resultSet.getString("teachers.tno"));
                    vector.add(resultSet.getString("teachers.nic"));
                    vector.add(resultSet.getString("teachers.fname"));
                    vector.add(resultSet.getString("teachers.lname"));
                    vector.add(resultSet.getString("teachers.contact_no"));
                    vector.add(resultSet.getString("teachers.dob"));
                    vector.add(resultSet.getString("gender.name"));
                    vector.add(resultSet.getString("teachers.password"));
                    vector.add(resultSet.getString("teachers.email"));
                    vector.add(resultSet.getString("teachers.registered_date"));
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "T_No", "NIC", "First Name", "Last Name", "Contact Number", "Birthday", "Gender", "Password", "Email Address", "Registered Date", "Status"
            }
        ) {
        ...
        jComboBox3.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "T_No ASC", "T_No DESC", "NIC ASC", "NIC DESC", "First Name ASC", "First Name DESC", "Last Name ASC", "Last Name DESC", "Contact Number ASC", "Contact Number DESC", "Birthday ASC", "Birthday DESC", "Gender ASC", "Gender DESC", "Password ASC", "Password DESC", "Email Address ASC", "Email Address DESC", "Registered Date ASC", "Registered Date DESC", "Status ASC", "Status DESC" }));
        ...
        if (jTable1.getSelectedRow() != -1) {
            ...
            try {
                ...
                ResultSet resultSet = MySQL.executeSearch("SELECT * FROM `teachers` "
                        + "WHERE `tno`='" + String.valueOf(jTable1.getValueAt(row, 0)) + "'");

                if (resultSet.next()) {

                    filename = resultSet.getString("image_path");
                    filepath = "Institute_Images/Teacher_Images/" + filename;
            ...
            if (evt.getClickCount() == 2) {
                ...
                new AddressView(null, true, String.valueOf(jTable1.getValueAt(row, 2)), String.valueOf(jTable1.getValueAt(row, 3)), String.valueOf(jTable1.getValueAt(row, 1)), "2").setVisible(true);
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        if (jTable1.getRowCount() > 0) {
            ...
            try {
                ...
                InputStream s = this.getClass().getResourceAsStream("/com/sahan/adyapana/reports/ai_teachers.jasper");
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        if (jTextField1.getText().isEmpty()) {
            ...
            JOptionPane.showMessageDialog(this, "Please Enter Teacher's NIC Number.",
                    "Warning", JOptionPane.WARNING_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Please Enter Teacher's Birthday.",
                    "Warning", JOptionPane.WARNING_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Please Select Teacher's Gender.",
                    "Warning", JOptionPane.WARNING_MESSAGE);
            ...
            JOptionPane.showMessageDialog(this, "Please add a teacher image.",
                    "Warning", JOptionPane.WARNING_MESSAGE);
            jButton2.grabFocus();

        } else if (!new File(filepath).exists()) {

            JOptionPane.showMessageDialog(this, "Image Uploading Error! \nPlease select the teacher image again.",
                    "Error", JOptionPane.ERROR_MESSAGE);
            ...
            try {
                ...
                ResultSet resultSet1 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `nic`='" + nic + "'");
                ...
                if (resultSet1.next()) {
                    ...
                    ResultSet resultSet4 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `contact_no`='" + contact_no + "'");

                    if (resultSet4.next()) {

                        JOptionPane.showMessageDialog(this, "This Mobile Number already registered. "
                                + "\nPlease Change Mobile Number and Try again.",
                                "Error", JOptionPane.ERROR_MESSAGE);
                        ...
                        ResultSet resultSet2 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `email`='" + email + "'");
                        ...
                        if (resultSet2.next()) {
                            ...
                            if (!new File("Institute_Images/Teacher_Images").exists()) {
                                new File("Institute_Images/Teacher_Images").mkdirs();
                            }

                            // Create Path objects
                            Path sourcePath = Paths.get(filepath);
                            String copy_file_path = "Institute_Images/Teacher_Images/" + filename;
                            Path destinationPath = Paths.get(copy_file_path);

                            // Ensure the destination directory exists
                            File destinationDir = new File("Institute_Images/Teacher_Images");
                            ...
                            ResultSet resultSet3 = MySQL.executeSearch("SELECT COUNT(`tno`) AS 'c' FROM `teachers`");
                            ...
                            if (resultSet3.next()) {
                                ...
                                // File renamed
                                if (new File(copy_file_path).renameTo(new File("Institute_Images/Teacher_Images/" + filename))) {

                                    MySQL.executeIUD("INSERT INTO `teachers` (`fname`,`lname`,`email`,`dob`,`nic`,`contact_no`,`password`,"
                                            + "`registered_date`,`gender_id`,`status_id`,`image_path`,`type_id`) VALUES('" + firstName + "','" + lastName + "','" + email + "',"
                                    ...
                                    Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_CENTER, 2500L, "New Teacher registered Successfully!");

                                } else {
                                    JOptionPane.showMessageDialog(this, "Teacher Image uploading fail. \nPlease try again!",
                                            "Error", JOptionPane.ERROR_MESSAGE);
                                }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if (jTable1.getSelectedRow() == -1) {
            ...
            if (jTextField1.getText().isEmpty()) {
                ...
                String tno = String.valueOf(jTable1.getValueAt(jTable1.getSelectedRow(), 0));
                String nic = jTextField1.getText();
                String firstName = jTextField2.getText();
                String lastName = jTextField3.getText();
                String contact_no = jTextField4.getText();
                ...
                if (nic.equals(selected_nic) && firstName.equals(selected_firstName) && lastName.equals(selected_lastName)
                        ...
                        && email.equals(selected_email) && filepath.equals("Institute_Images/Teacher_Images/" + filename)) {
                    ...
                    try {
                        ...
                        ResultSet resultSet1 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `nic`='" + nic + "' AND NOT `tno`='" + tno + "'");
                        ...
                        if (resultSet1.next()) {
                            ...
                            ResultSet resultSet4 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `contact_no`='" + contact_no + "' AND NOT `tno`='" + tno + "'");
                            ...
                            if (resultSet4.next()) {
                                ...
                                ResultSet resultSet2 = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `email`='" + email + "' AND NOT `tno`='" + tno + "'");
                                ...
                                if (resultSet2.next()) {
                                    ...
                                    if (option == JOptionPane.YES_OPTION) {
                                        ...
                                        if (!filepath.equals("Institute_Images/Teacher_Images/" + filename)) {
                                            ...
                                            if (resultSet3.next()) {
                                                ...
                                                if (new File(copy_file_path).renameTo(new File("Institute_Images/Teacher_Images/" + filename))) {
                                                    ...
                                                    ResultSet rs = MySQL.executeSearch("SELECT * FROM `teachers` WHERE `tno`='" + tno + "'");

                                                    if (rs.next()) {

                                                        if (new File("Institute_Images/Teacher_Images/" + rs.getString("image_path")).exists()) {

                                                            new File("Institute_Images/Teacher_Images/" + rs.getString("image_path")).delete();
                                        ...
                                        if (!canUpdate1) {
                                            ...
                                            MySQL.executeIUD("UPDATE `teachers` SET `fname`='" + firstName + "',`lname`='" + lastName + "',`email`='" + email + "',"
                                            ...
                                            Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_CENTER, 2500L, "Teacher Details updated Successfully!");
                                        }
        ...
        if (jTable1.getSelectedRow() == -1) {
            ...
            if (option == JOptionPane.YES_OPTION) {
                ...
                try {
                    ...
                    MySQL.executeIUD("UPDATE `teachers` SET `status_id`='" + status_id + "' WHERE `tno`='" + tno + "'");

                    Reset();
                    LoadTeachers();

                    Notifications.getInstance().show(Notifications.Type.SUCCESS, Notifications.Location.TOP_CENTER,
                            2500L, "Teacher's status updated Successfully!");
